...
template <int START, int COUNT> uint64_t getBits(uint64_t v) {
  if (START)
    v = (v >> START);
...
public:
  ...
  Bases getBasesFromReference(const size_t referencePosition,
                              const unsigned readLength) const;
...
TEST_F(MapperFixture, explore) {
  ...
  const Bases bases =
      getBasesFromReference(READ_REFERENCE_POSITION, readLength);
  ...
  std::cerr << "\n    -------------------\n        position: "
            << READ_REFERENCE_POSITION << " (" << std::hex
            << READ_REFERENCE_POSITION << std::dec
            << "): " << basesToString(bases) << std::endl;
  // std::cerr <<  "bases copy: " << basesToString(Bases(bases)) << std::endl;
  // std::cerr <<  "read: " << bases2bpbToString(read.getBases()) << std::endl;
  // std::cerr << "Generating seed offsets for " << readLength << " " <<
  // seedLength << " " << SEED_PERIOD << " " << SEED_PATTERN << " " <<
  // FORCE_LAST_N_SEEDS << std::endl;
  const auto seedOffsets = Seed::getSeedOffsets(
      readLength, seedLength, SEED_PERIOD, SEED_PATTERN, FORCE_LAST_N_SEEDS);
  // for (auto offset: seedOffsets) std::cerr << " " << offset;
  // std::cerr << std::endl;
  ChainBuilder chainBuilder(4.0);
#if 1
  std::cerr << "\n      ++++++++++++++++++++++++++ Seed offsets... +++++++++++ "
            << std::endl;
  for (auto offset : seedOffsets) {
    ...
    const auto hash = hashtable->getPrimaryHasher()->getHash64(
        seedIsReverseComplement ? reverseData : forwardData);
    ...
    const auto hashThreadId =
        hashtable->getThreadIdFromVirtualByteAddress(virtualByteAddress);

    std::cerr << "  hits for offset " << std::setw(3) << offset << ": "
              << std::setfill('0')
              << (seedIsReverseComplement ? "reverse" : "forward")
              << " data: " << std::setw(14) << std::hex
              << (seedIsReverseComplement ? reverseData : forwardData)
              << " hash: " << std::setw(14) << hash
              << " thread Id: " << std::setw(2) << (unsigned)hashThreadId
              << " bucketIndex: " << std::setw(8) << bucketIndex
              << " matchBits: " << std::setw(8) << matchBits << " orientation: "
              << (seedIsReverseComplement ? "REVERSE" : "FORWARD") << std::dec
    ...
    for (const auto &interval : extendTableIntervals) {
      std::cerr << "  " << std::dec << std::setw(10) << interval.getStart()
                << ":" << std::setw(10) << interval.getLength();
    }
    std::cerr << std::endl;
    std::cerr << "     hits count for offset " << std::setw(3) << offset << ": "
              << std::dec << hashRecords.size();
    unsigned extendCount = 0;
    for (const auto &hit : hashRecords) {
      std::cerr << "  " << std::hex << std::setw(8) << (hit.getValue() >> 32)
                << ":" << std::setw(8) << (hit.getValue() & 0xFFFFFFFF);
    }
    std::cerr << std::endl;
    for (const auto &hit : hashRecords) {
      if (HashRecord::EXTEND == hit.getType()) {
        ++extendCount;
        while (!hashRecords.empty() &&
               (HashRecord::EXTEND == hashRecords.front().getType())) {
          const uint64_t primaryMask =
              (static_cast<uint64_t>(1) << hashtable->getPrimaryCrcBits()) - 1;
          const uint64_t secondaryMask =
              (static_cast<uint64_t>(1) << hashtable->getSecondaryCrcBits()) -
              1;
          ...
          std::cerr << "        " << std::hex << std::setw(8)
                    << (hit.getValue() >> 32) << ":" << std::setw(8)
                    << (hit.getValue() & 0xFFFFFFFF)
                    << ": Trying extension from " << std::dec
                    << fromHalfExtension << " to half-extension "
                    << toHalfExtension << "..." << std::endl;
          if (seed.isValid(fromHalfExtension + halfExtension)) {
            const unsigned halfPaddingBits = (6 - halfExtension) * 2;
            const uint64_t data = seed.getExtendedData(
                fromHalfExtension, toHalfExtension, seedIsReverseComplement);
            // const auto extendBases = seed.getExtendedData(fromHalfExtension,
            // toHalfExtension, seedIsReverseComplement) << halfPaddingBits;
            const uint64_t extendBases = data << halfPaddingBits;
            const uint64_t shiftedExtensionId = extendRecord.getExtensionId()
                                                << 24;
            const uint64_t shiftedExtensionIdBin = ((hash & 0x7f) << (24 + 18));
            const auto extendedKey =
                shiftedExtensionIdBin | shiftedExtensionId | extendBases;
            const auto extendedHash =
                addressSegment |
                hashtable->getSecondaryHasher()->getHash64(extendedKey);
            hashtable->getHits(extendedHash, true, hashRecords,
                               extendTableIntervals, true);
            std::cerr << "             extendedKey: " << std::hex << extendedKey
                      << ": extendedHash: " << std::hex << extendedHash
                      << std::dec << ": Found " << hashRecords.size()
                      << " records:";
            for (const auto &hit : hashRecords) {
              std::cerr << "  " << std::hex << std::setw(8)
                        << (hit.getValue() >> 32) << ":" << std::setw(8)
            ...
            std::cerr << std::endl;
            std::cerr << "             Found " << extendTableIntervals.size()
                      << " intervals:";
            for (const auto &interval : extendTableIntervals) {
              std::cerr << "  " << std::dec << std::setw(10)
                        << interval.getStart() << ":" << std::setw(10)
                        << interval.getLength();
            }
            std::cerr << std::endl;
  ...
  const auto compare = [](const dragenos::map::SeedChain &lhs,
                          const dragenos::map::SeedChain &rhs) -> bool {
  ...
  for (const auto &seedChain : chainBuilder) {
    ...
    std::cerr << "SeedChain: RC: " << seedChain.isReverseComplement()
              << " RS: " << seedChain.hasOnlyRandomSamples()
              << " size: " << seedChain.size() << std::endl;
    for (const auto &seedPosition : seedChain) {
      const auto &seed = seedPosition.getSeed();
      std::cerr << "    " << std::setw(10) << std::dec
                << seedPosition.getReferencePosition() << ": seed ("
                << std::setw(3) << seed.getReadPosition() << ": "
                << seed.getPrimaryLength() << " + 2*" << std::setw(2)
                << seedPosition.getHalfExtension() << ")"
                << ": from " << std::setw(10)
                << seedPosition.getFirstProjection(reverseComplement) << " to "
                << std::setw(10)
                << seedPosition.getLastProjection(reverseComplement)
                << std::endl;
    }
  }
}
...
MapperFixture::Read::Bases
MapperFixture::getBasesFromReference(const size_t referencePosition,
                                     const unsigned readLength) const {
  ...
  for (unsigned i = 0; readLength > i; ++i) {
    const unsigned char base =
        referenceSequence.getBase(i + READ_REFERENCE_POSITION);
...
void MapperFixture::SetUpTestCase() {
  ...
  read.init(Read::Name(3, 'A'), Read::Bases(READ_LENGTH, 0),
            Read::Qualities(READ_LENGTH, 30), 0, 0);
}
